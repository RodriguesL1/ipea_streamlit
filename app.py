...
st.write("No período de 2021 a 2022 teve uma recuperação da demanda global após a pandemia, combinado com algumas restrições",
         ...
         "E as tenções geopoliticas, como por exemplo o conflito entre a Rússia e a Ucrânia.")
st.write("Esses fatores ressultaram em uma alta volatilidade e em preços recordes (perdendo só para a crise de 2008), influenciando diretamente a inflação global e gerando "
...
feriados_dias_uteis = [
    pd.to_datetime(f"{feriado}/{ano}", format='%d/%m/%Y').strftime('%d/%m/%Y')
    for ano in range(2019, 2025)
    for feriado in feriados_nacionais
    if pd.to_datetime(f"{feriado}/{ano}", format='%d/%m/%Y').weekday() < 5
]
